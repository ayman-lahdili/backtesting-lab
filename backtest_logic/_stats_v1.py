...
def performance(name: str, symbol: list, data: dict, equity: pd.Series, tradebook: pd.DataFrame, freq, min_bars=0, Rfr=0):
    ...
    if 'min' in freq:
        ...
        min_lookback = str(min_bars * freq) + 'hour'
    elif freq == 'daily':
        freq = 6.5 * 60
    ...
    for sym in symbol:
        data[sym] = data[sym].iloc[min_bars: data[sym].shape[0]]
    ...
    # benchmark_ret = np.zeros_like(strategy_ret)
    # for sym in symbol:
    #     ratio = 1/len(symbol)
    #     ret = (np.log(data[sym]['close']).diff() * ratio).dropna()
    #     benchmark_ret += ret
    ...
    #start
    abs_start_date = abs_start_date
    start_date = data[sym]['date'].iloc[0]
    end_date = data[sym]['date'].iloc[-1]
    ...
    print('shape',data[sym]['date'].shape[0])
    print(end)
    dd_start = parser.parse(data[sym].loc[start, 'date'])
    dd_end = parser.parse(data[sym].loc[end, 'date'])
